...
public class ScoreUtils {
	...
	private static long HALF_HOUR_MILLIS = 1000 * 60 * 30;
	...
	public static float getNewsScoreFactor(long now, NumericDocValues numericDocValues, int docId) {
		long time = numericDocValues.docID();
		float factor = 1;
		int day = (int) (time / DAY_MILLIS);
		int nowDay = (int) (now / DAY_MILLIS);
		...
		} else if (now - time <= HALF_HOUR_MILLIS && now >= time) {
